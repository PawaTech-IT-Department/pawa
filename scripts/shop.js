// Top-level variables for global access
let allProducts = [];
    ...
    productsToDisplay.forEach((product) => {
      productHTML += `
        <div class="product--card">
          ...
          <button class="btn btn--primary add-to-cart js-add-to-cart" data-product-id="${product.id}">Add to Cart</button>
        </div>
      `;
    });
  }
...
function renderPagination() {
  ...
  paginationHTML += `<a href="#" class="js-pagination-link ${currentPage === 1 ? "disabled" : ""}" data-page="${currentPage > 1 ? currentPage - 1 : 1}">&lt;</a>`;
  for (let i = 1; i <= totalPages; i++) {
    paginationHTML += `<a href="#" class="js-pagination-link ${i === currentPage ? "active" : ""}" data-page="${i}">${i}</a>`;
  }
  paginationHTML += `<a href="#" class="js-pagination-link ${currentPage === totalPages ? "disabled" : ""}" data-page="${currentPage < totalPages ? currentPage + 1 : totalPages}">&gt;</a>`;
...
function updateCategoryCounts() {
  ...
  allProducts.forEach((product) => {
    categoryCounts.all++;
    if (isProductInCategory(product, "laptop")) categoryCounts.laptops++;
    else if (isProductInCategory(product, "printer")) categoryCounts.printers++;
    else if (isProductInCategory(product, "projector")) categoryCounts.projectors++;
    else if (isProductInCategory(product, "tablet")) categoryCounts.tablets++;
    else categoryCounts.accessories++;
  });
...
function isProductInCategory(product, category) {
  ...
  if (categoryLower === "all") return true;
...
document.addEventListener("DOMContentLoaded", async function () {
  productGrid = document.querySelector(".js--product--grid");
  minPriceSlider = document.querySelector(".js-min-price-slider");
  maxPriceSlider = document.querySelector(".js-max-price-slider");
  searchInput = document.querySelector(".js-search-input");
  sortSelect = document.querySelector(".js-sort-select");
  categoryRadios = document.querySelectorAll(".js-category-radio");
  minPriceLabel = document.querySelector(".js-min-price-label");
  maxPriceLabel = document.querySelector(".js-max-price-label");
  try {
